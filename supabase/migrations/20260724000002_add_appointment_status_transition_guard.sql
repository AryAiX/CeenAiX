-- New validated RPC: the only way clinic staff can change an appointment's status
-- going forward. Enforces: completed/cancelled/no-show are terminal (locked);
-- scheduled -> confirmed -> in-progress -> completed is forward-only;
-- cancelled/no-show are reachable as an exit from any active state.
...
-- Retire the two blanket clinic UPDATE policies. Clinic's only confirmed write
-- path to an existing appointment is status changes, which now goes exclusively
-- through the RPC above (SECURITY DEFINER, so it isn't affected by this).
