...
BEGIN
  ...
  -- Get the instrument used for this order
  SELECT DISTINCT instrument INTO used_instrument
  ...
  -- Set instrument back to not running
  IF used_instrument IS NOT NULL THEN
    UPDATE public.lab_portal_equipment
    SET is_running = false,
        updated_at = now()
    WHERE lab_id = current_lab
      AND department = 'laboratory'
      AND name = used_instrument;
  END IF;
