...
| Supabase Auth | Branded auth email HTML (all auth mailers, all roles) | Prod | Release workflow → `scripts/sync-prod-auth-platform.sh` | done (CI) | **One shared layout for every role.** Prod sync runs on every `main` release. Dev: run `./scripts/sync-auth-email-templates.sh lgfaucsfiyxvmsghnpey --site-url http://localhost:5173` when templates change. |
...
| Supabase Auth | Site URL for auth emails and redirects | Prod | `scripts/prod-platform-config.env` + Release workflow | done (CI) | Prod `site_url` is `https://www.ceenaix.com` (PATCHed each release). Dev: `--site-url http://localhost:5173` via sync script. |
| Supabase Auth | Redirect URL allow-list for final domains | Prod | `scripts/prod-platform-config.env` + Release workflow | done (CI) | `PROD_URI_ALLOW_LIST` includes www, apex, Vercel previews, localhost. |
