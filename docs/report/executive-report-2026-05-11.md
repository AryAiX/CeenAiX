...
For the week ending 2026-05-11, CeenAiX shipped **six PRs that brought the last three hosted-parity portals (Admin, Insurance, Lab wrap-up) to demo-credible, completed the admin onboarding round-trip, added a Playwright E2E suite, and closed a 120-fix codebase audit**. All six operational portals — patient, doctor, pharmacy, lab, insurance, admin — are now hosted-parity, Supabase-backed, and covered by end-to-end browser tests.

The largest functional change was the **Admin Platform Portal rewrite** (PR #32) — new shell + shared primitives + 11 new tables + 5 new SECURITY-DEFINER RPCs + 5 typed hooks powering Dashboard, Patients, Doctors, Organizations, Insurance, and AI Analytics. In parallel, the **Insurance Operator Portal** was rewritten across all nine tabs (PR #34) data-bound through an expanded `use-insurance-portal.ts` hook with no schema change. The **Lab & Radiology portal** was wrapped (PR #31) by refactoring `EquipmentCard` for department-specific layouts and dynamic status labels, switching `formatTat` to decimal hours, fixing sidebar facility/footer/badge semantics, and seeding three inbox rows for Lab and Imaging Orders.

The **admin onboarding round-trip** (PR #35) wired previously-inert header buttons (Analytics, CSV Export, Add Doctor / Register Patient / Onboard Insurer) and added an in-app `OnboardOrganizationModal` backed by a new `admin_create_organization` RPC so super-admins can create hospitals, clinics, labs, pharmacies, and insurance tenants without leaving `/admin/*`; the Register page's Full Name input was also hardened against browser autofill. The **Playwright E2E suite** (PRs #33/#36, documented in the [companion weekly report](../weekly-report/weekly-report-2026-05-11-e2e-scenario-tests.md)) ships **124 passing browser tests**, including **23 cross-role scenario tests** and explicit negative-case coverage. Finally, the long-running `bugs` branch closed **120 categorized bug fixes** across ten functional areas — localization, timezones, numeric defenses, nullish-coalescing corrections, and the onboarding redirect loop.
